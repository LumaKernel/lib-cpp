...
問題によると思いますが，重さが0以下のものは全て採用価値が0以下のものは全て捨てることで $1 \leq x_i, y_i$ を仮定します
...
ここでわかりやすさのために，制約にある条件を**制約条件**と呼ぶことにします．これは絶対に守らなければいけないものです．これを守っているある解 (つまり $c$ という数列) を**実行可能解**
...
>  また $y_p y_q$ 減らして同じ分だけ増えているので，結果的により少ない重さで

> 定義 : 以下の条件を最適条件Xとする
> $p < q$ で $z_p - c_p \geq Y$ かつ $c_q \geq Y$ となるような $p, q$ が存在しない

上記の操作は有限回しか行なえません．よっていつかは最適条件を満たす実行可能解になります
