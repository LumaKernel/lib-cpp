...
各 $i$ について$z_i = 1$ である場合には 01ナップサック問題 と呼ばれます
...
申し訳ないのですが，上の4つについては前提知識とします 

実際は4つめだけで大丈夫です．4つめを**アルゴリズムB**と呼ぶことにします
...
それはそう (入れることができるものが残っているなら入れたほうが良い)

> 最適条件Xを満たす最適解が存在する

これは任意の最適解について **最適条件Xへと近づける操作** を有限回行うことで最適条件Xを満たす解へとたどり着けるからです

> アルゴリズムAは全ての最適条件Xを満たす極大な実行可能解を探索している
...
"最後にY個以上使った" "最初にY個以上余った"と言えるようなインデックスを $i, j$ と置くことにします (最適条件Xを満たすので $i \leq j$)
